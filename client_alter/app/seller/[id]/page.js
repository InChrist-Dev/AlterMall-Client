...
const [seller, setSeller] = useState([]);
  ...
  const fetchData = async () => {
    try {
      const response = await fetch(`http://localhost:8000/user/seller/detail/${props.params.id}`);
...
const nextSlide = () => {
  ...
  return (
    <div className={styles.container}>
      <div className={styles.background} style={{background: `linear-gradient(rgba(0,0,0 , 0), rgba(0, 0, 0, 1)), url('http://localhost:8000/${sellerDetail.img}') center/cover`}}></div>
      <div className={styles.content}>
        <div style={{ display: 'flex', justifyContent: 'space-between' }}>
          <div>
            <div style={{ display: 'flex'}}>
            <img className={styles.logo} src={`http://localhost:8000/${sellerDetail.logo}`}></img>
          ...
          <div className={styles.episodeSlider} style={{ transform: transformValue }}>
            {products.map((products, i) => (
              <div key={i} className={styles.episodeCard}>
                ...
                <img className={styles.episodeImage} src={`http://localhost:8000/${products.img}`} />
</a>
              </div>
            ))}
          </div>
